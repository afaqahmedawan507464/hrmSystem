       ...
       <div class="card card-solid">
        ...
        @endif
          ...
          <div class="card-body p-5">
             <div class="row">
                @foreach ($userListApprovalRequest as $item)
                   <div class="col-12 col-sm-6 col-md-4 d-flex align-items-stretch flex-column">
                   <div class="card bg-light d-flex flex-fill">
                      {{-- @if ($item->online_status == 1)
                      <div class="card-header text-muted border-bottom-0 bg-success">
                         <p class="text-white status_data">Online</p>
                      </div>
                      @else
                      <div class="card-header text-muted border-bottom-0 bg-secondary">
                         <p class="text-white status_data">Offline</p>
                      </div>
                      @endif --}}
                      ...
                      <div class="card-footer">
                        <div class="text-right">
                           <a href="{{ route('hraccountRejected',$item->id) }}" class="btn btn-sm bg-danger text-white">
                           Dismiss User
                           </a>
                           <a href="{{ route('hraccountApprove',$item->id) }}" class="btn btn-sm btn-primary">
                           <i class="fas fa-user"></i> Accept Account
                           </a>
                        </div>
                     </div>
                      @else
                      <div class="card-footer">
                        <div class="text-right">
                           <a href="{{ route('accountRejected',$item->id) }}" class="btn btn-sm bg-danger text-white">
                           Dismiss User
                           </a>
                           <a href="{{ route('accountApprove',$item->id) }}" class="btn btn-sm btn-primary">
                           <i class="fas fa-user"></i> Accept Account
                           </a>
                        </div>
                     </div>
